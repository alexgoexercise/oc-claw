"""
Re-pack a companion raw atlas into clean 192x208 hatch-pet cells.
...
Usage:
  python3 pack_companion_atlas.py --id pep --name Pep \\
    --desc "A cheeky pixel-art desktop companion of Pep in a deep blue tee." \\
    --raw scripts/_pep_build/pep_atlas_raw.png
"""
...
def write_package(atlas: Image.Image, out_dir: Path, pet_id: str, display_name: str, description: str) -> None:
...
def write_audit(atlas: Image.Image, audit_dir: Path) -> None:
    ...
    for row, need in enumerate(FRAME_COUNTS):
        ...
        strip.save(audit_dir / f"fixed_row{row}_strip.png")
    print("audit strips written to", audit_dir)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Pack companion raw atlas into oc-claw pet format")
    ap.add_argument("--id", required=True, help="pet id / folder name, e.g. pep")
    ap.add_argument("--name", required=True, help="display name")
    ap.add_argument("--desc", default="", help="pet.json description")
    ap.add_argument("--raw", required=True, help="path to raw atlas png (relative to frontend/ or absolute)")
    ap.add_argument("--build-dir", default="", help="optional build/audit dir (default scripts/_<id>_build)")
    args = ap.parse_args()

    raw_path = Path(args.raw)
    if not raw_path.is_absolute():
        raw_path = ROOT / raw_path
    build_dir = Path(args.build_dir) if args.build_dir else ROOT / f"scripts/_{args.id}_build"
    if not build_dir.is_absolute():
        build_dir = ROOT / build_dir
    out_dir = ROOT / "public/assets/custom" / args.id
    audit_dir = build_dir / "audit"
    desc = args.desc or f"A pixel-art desktop companion of {args.name} in a deep blue tee."

    raw = Image.open(raw_path)
    print("raw size", raw.size, "pet", args.id)
    rows = extract_rows(raw)
    atlas = compose(rows)
    assert atlas.size == (ATLAS_W, ATLAS_H)
    write_package(atlas, out_dir, args.id, args.name, desc)
    write_audit(atlas, audit_dir)
    verify(atlas)
